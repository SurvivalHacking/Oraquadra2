// OraQuadra V1.2.6 PE -  By Davide Gatti SURVIVAL HACKING  www.survivalhacking.it e Marco Prunca
...
#include <math.h>        // Per funzioni matematiche (sqrt, atan2, cos, sin)
...
#define MODE_DROP 11    // Effetto goccia con onde concentriche
#define MODE_RAINBOW 12 // Effetto arcobaleno graduale
#define MODE_ARKANOID 13 // Effetto pallina rimbalzante tipo Arkanoid
#define MODE_CHRISTMAS 14 // Effetto Natalizio con colori festivi e sparkle
#define MODE_SNOWFALL 15  // Effetto neve che cade con cristalli ghiacciati
#define MODE_LASER 16     // Effetto raggi laser dal basso che rivelano l'orario
#define MODE_FUOCO 17     // Effetto fiamme animate dal basso con orario visibile
#define MODE_BLACKHOLE 18 // Effetto black hole con particelle risucchiate verso il centro
#define MODE_FIRE1 19     // Effetto Fire2012 stile WLED - simulazione fuoco realistica
#define MODE_FIREWORK 20  // Effetto Fireworks 1D stile WLED - fuochi d'artificio esplosivi
#define MODE_DRIFT 21     // Effetto Drift stile WLED - particelle che driftano lentamente
#define NUM_MODES   22  // Numero di modalità di visualizzazione (MODE_FADE a MODE_DRIFT)
#define NUM_PRESETS 29  // Numero totale di preset (0-28)  
...
// Strutture e costanti per effetto CHRISTMAS
...
// Strutture e costanti per effetto SNOWFALL
...
// Strutture e costanti per effetto LASER
...
// Strutture e costanti per effetto FUOCO
#define FUOCO_SPEED 40            // Velocità dell'effetto (ms tra i frame)
#define FUOCO_COOLING 55          // Quanto velocemente si raffredda il fuoco (più alto = meno intenso)
#define FUOCO_SPARKING 120        // Probabilità di nuove scintille (0-255)
#define FUOCO_HEIGHT 14           // Altezza massima fiamme (in pixel dall'alto) - lascia 2 righe libere in basso
#define FUOCO_MAX_DURATION 15000  // Durata massima effetto fuoco (15 secondi)
// Strutture e costanti per effetto BLACKHOLE
...
// Strutture e costanti per effetto FIRE1 (Fire2012 style)
#define FIRE1_SPEED 30            // Velocità dell'effetto (ms tra i frame)
#define FIRE1_COOLING 50          // Quanto velocemente si raffredda (più alto = meno intenso)
...
#define FIRE1_HEIGHT 10           // Altezza effetto in righe (metà matrice + 2 = 10 righe)
// Strutture e costanti per effetto FIREWORK
...
#define MAX_FIREWORKS 6           // Numero massimo di fuochi d'artificio contemporanei
#define MAX_PARTICLES 80          // Numero massimo di particelle totali
// Strutture e costanti per effetto DRIFT
...
#define DRIFT_BLUR_AMOUNT 172     // Quantità di blur/fade (0-255, più alto = più fade)
...
#define EEPROM_SCROLL_SPEED_ADDR 44
...
// Nuova posizione per durata orario dopo scorrimento
#define EEPROM_SCROLL_CLOCK_DURATION_ADDR 350
// Nuova posizione per effetto da visualizzare tra gli scorrimenti
#define EEPROM_SCROLL_BETWEEN_EFFECT_ADDR 351
// Flag per abilitare/disabilitare l'effetto tra scorrimenti
#define EEPROM_SCROLL_BETWEEN_EFFECT_ENABLED_ADDR 352
// Impostazioni per modalità: 250-349 (17 modalità × 5 byte = 85 byte)
// Ogni modalità: Marker(1) + R(1) + G(1) + B(1) + Rainbow(1) = 5 byte
...
// Stati per il testo scorrevole
enum ScrollTextState {
    SCROLL_STATE_SCROLLING,
    SCROLL_STATE_PAUSE,
    SCROLL_STATE_CLOCK
};
ScrollTextState scrollTextState = SCROLL_STATE_SCROLLING;
...
uint8_t scrollClockDuration = 5;          // Durata in secondi dell'orologio a fine scorrimento (default 5)
uint32_t scrollClockStart = 0;            // Quando è iniziata la visualizzazione dell'orologio
bool scrollClockActive = false;           // Se siamo in fase di visualizzazione orologio
uint8_t scrollBetweenEffect = MODE_DIGITALE; // Effetto da visualizzare tra gli scorrimenti
bool scrollBetweenEffectEnabled = true;   // Abilita/disabilita l'effetto tra scorrimenti
...
struct ClockColors {
   ...
   CRGB getColor() const {
       if (textRainbowMode) {
           // In modalità rainbow, restituisce un colore che cambia nel tempo
   ...
   // Colore rainbow basato su posizione (per testo scorrevole)
...
// ============================================================================
// Salva le impostazioni correnti (colore, rainbow) per la modalità specificata
void saveModeSettings(uint8_t preset) {
    ...
    EEPROM.commit();
    Serial.print("Salvate impostazioni per preset ");
    Serial.print(preset);
    Serial.print(" - R=");
...
// Carica le impostazioni salvate per la modalità specificata
void loadModeSettings(uint8_t preset) {
    if (preset >= NUM_PRESETS) return;
    uint16_t baseAddr = EEPROM_MODE_SETTINGS_START + (preset * EEPROM_MODE_SETTINGS_SIZE);
    uint8_t marker = EEPROM.read(baseAddr);
    // Verifica se questa modalità ha impostazioni salvate
    if (marker != EEPROM_MODE_MARKER) {
        // Nessuna impostazione salvata, mantieni i valori di default di applyPreset
        Serial.print("Nessuna impostazione salvata per preset ");
    ...
    Serial.print("Caricate impostazioni per preset ");
    Serial.print(preset);
    Serial.print(" - Colore: R=");
    Serial.print(r);
    Serial.print(" G=");
    Serial.print(g);
    Serial.print(" B=");
...
// Font 3x5 per caratteri piccoli - definito in PROGMEM per risparmiare RAM
...
// Funzione per ottenere un carattere dal font piccolo 3x5
...
// Funzione per ottenere un carattere dal font medio 5x7
...
uint16_t xyToLED(uint8_t x, uint8_t y) {
  // La matrice è mappata in serpentina, mais invertita rispetto allo standard
  if (y % 2 == 0) {
...
// Funzione per visualizzare testo scorrevole con dimensione font dinamica
void scrollText(String text, CRGB color, uint16_t scrollSpeed) {
  ...
  // Parametri in base alla dimensione del font
...
// Funzione helper per costruire il testo completo con data/ora
String buildScrollDisplayText() {
  String parts[3];  // Data, Ora, Testo
  int numParts = 0;
  // Aggiungi data se abilitata (formato: LUNEDI 02 DICEMBRE 2025)
  if (scrollShowDate) {
    int giornoSett = weekday() - 1;
    ...
    String dateStr = "";
    dateStr += giorniSettimana[giornoSett];
    dateStr += " ";
    if (day() < 10) dateStr += "0";
    dateStr += String(day());
    dateStr += " ";
    dateStr += mesiAnno[meseIdx];
    dateStr += " ";
    dateStr += String(year());
    parts[numParts++] = dateStr;
  }
  // Aggiungi ora se abilitata (senza secondi per evitare reset continuo dello scroll)
  if (scrollShowTime) {
    char timeStr[6];
    sprintf(timeStr, "%02d:%02d", currentHour, currentMinute);
    parts[numParts++] = String(timeStr);
  }
  // Aggiungi testo personalizzato (solo se abilitato)
  if (scrollShowText && customScrollText.length() > 0) {
    parts[numParts++] = customScrollText;
  }
  // Costruisci il risultato unendo le parti con " - "
  String result = "";
  for (int i = 0; i < numParts; i++) {
    if (i > 0) result += " - ";
    result += parts[i];
  }
  // Se tutto vuoto, metti uno spazio
  if (result.length() == 0) result = " ";

  return result;
}
// Versione NON bloccante del testo scorrevole per modalità custom
void initScrollText() {
  // Usa buildScrollDisplayText per includere data/ora se attivi
...
// Funzione per resettare lo stato del testo scorrevole
void resetScrollText() {
  scrollTextState = SCROLL_STATE_SCROLLING;
  scrollTextInitialized = false;
  scrollClockActive = false;
  scrollTextOffset = MATRIX_WIDTH;
  scrollPauseStart = 0;
  scrollClockStart = 0;
}
// Funzione per aggiornare lo stato di scorrimento
void updateScrollTextScrolling(uint32_t now) {
  if (!scrollTextInitialized) initScrollText();
  if (now - lastScrollTextStep < scrollTextSpeedMs) return;
  lastScrollTextStep = now;
  // Costruisci il testo completo con data/ora se abilitati
  String t = buildScrollDisplayText();
  t.toUpperCase();
  // Parametri in base alla dimensione del font
  int charWidth, charHeight, charSpacing, vertOffset;
  ...
  switch (scrollFontSize) {
    case 1: // Piccolo 3x5
      ...
      charHeight = 5;
      charSpacing = 4; // 3 + 1
      vertOffset = 5;  // Centra verticalmente (16-5)/2 = 5.5
      break;
    case 3: // Grande (font 5x7 scalato 2x = 10x14)
      charWidth = 5;
      charHeight = 7;
      charSpacing = 6;
      vertOffset = 1;  // Centra verticalmente (16-14)/2 = 1
  ...
  int effectiveCharSpacing = charSpacing * scale;
  // Aggiorna la larghezza totale per il ciclo
  ...
  for (int charIndex = 0; charIndex < textLength; charIndex++) {
    ...
    // Calcola il colore per questo carattere
    CRGB charColor;
    if (textRainbowMode) {
      uint8_t hue = (charIndex * 25) % 256;
      charColor = CHSV(hue, 255, 255);
    } else {
      charColor = clockColors.getColor();
    }
    if (scrollFontSize == 1) {
      // Font piccolo 3x5
      uint8_t charBitmap[3];
  ...
  FastLED.show();
  scrollTextOffset--;
    // Quando il testo esce completamente dallo schermo, passa alla pausa
  if (scrollTextOffset < -scrollTextTotalWidth) {
    if (scrollPauseSeconds > 0) {
      scrollTextState = SCROLL_STATE_PAUSE;
      scrollPauseStart = now;
    } else {
      scrollTextState = SCROLL_STATE_CLOCK;
      resetEffectForScrollText(scrollBetweenEffect); // Reset effetto prima di visualizzarlo
      scrollClockStart = now;
    }
    scrollTextInitialized = false;
  }
}
// Funzione per aggiornare lo stato di pausa
void updateScrollTextPause(uint32_t now) {
  // Aspetta il tempo di pausa configurato
  if (now - scrollPauseStart >= (uint32_t)scrollPauseSeconds * 1000) {
    scrollTextState = SCROLL_STATE_CLOCK;
    resetEffectForScrollText(scrollBetweenEffect); // Reset effetto prima di visualizzarlo
    scrollClockStart = now;
    scrollClockActive = true;
  }
}

// Forward declarations per effetti usati in updateScrollTextBetweenEffect
void updateMatrix();
void updateMatrix2();
...
// Funzione per resettare l'effetto prima di visualizzarlo tra gli scorrimenti
void resetEffectForScrollText(uint8_t effect) {
    switch (effect) {
        case MODE_MOTO:
            initMotoEffect();
            break;
...
// Funzione per aggiornare lo stato di visualizzazione effetto tra gli scorrimenti
void updateScrollTextBetweenEffect(uint32_t now) {
    // Se l'effetto tra scorrimenti è disabilitato
    if (!scrollBetweenEffectEnabled) {
        // Salta la pausa e torna subito allo scorrimento
        // L'ora/data saranno già incluse nel testo scorrevole se abilitati
        scrollTextState = SCROLL_STATE_SCROLLING;
        scrollClockActive = false;
        scrollTextOffset = MATRIX_WIDTH;
        scrollTextInitialized = false;
        ...
        // Mostra l'effetto selezionato per il tempo configurato
        switch (scrollBetweenEffect) {
        case MODE_DIGITALE:
        ...
        case MODE_FAST:
            ...
            break;
        default:
            // Fallback all'orologio digitale
            updateDigitaleEffect();
            break;
        }
    }

    // Quando scade il tempo, ricomincia lo scorrimento
    // Determina la durata in base allo stato
    uint32_t effectDuration;
    if (!scrollBetweenEffectEnabled) {
        // Se l'effetto è disabilitato, usa sempre scrollClockDuration per mostrare solo l'ora
        effectDuration = scrollClockDuration;
    } else {
        // FIRE1, FIREWORK e DRIFT durano sempre 15 secondi, gli altri effetti usano scrollClockDuration
        effectDuration = (scrollBetweenEffect == MODE_FIRE1 || scrollBetweenEffect == MODE_FIREWORK || scrollBetweenEffect == MODE_DRIFT) ? 15 : scrollClockDuration;
    }

    if (now - scrollClockStart >= effectDuration * 1000) {
        scrollTextState = SCROLL_STATE_SCROLLING;
        scrollClockActive = false;
        scrollTextOffset = MATRIX_WIDTH;
        scrollTextInitialized = false;
...
// Funzione principale per aggiornare il testo scorrevole
void updateScrollText() {
  uint32_t now = millis();
    
    switch (scrollTextState) {
    case SCROLL_STATE_SCROLLING:
      updateScrollTextScrolling(now);
      break;
    case SCROLL_STATE_PAUSE:
      updateScrollTextPause(now);
      break;
    case SCROLL_STATE_CLOCK:
      updateScrollTextBetweenEffect(now); // MODIFICATO: usa la nuova funzione
      break;
  }
}
// =============================================================================
...
// =============================================================================
// Forward declarations aggiunte per stabilità compilazione
...
void handleTest() {
    Serial.println("handleTest chiamata");
    server.send(200, "text/html", "<html><body><h1>Test OK</h1></body></html>");
}
...
void setupWebServer() {
    ...
    server.on("/setScrollClockDuration", HTTP_POST, handleSetScrollClockDuration); // Nuovo endpoint per durata orologio
    server.on("/setScrollBetweenEffect", HTTP_POST, handleSetScrollBetweenEffect); // Nuovo endpoint per effetto tra scorrimenti
    server.on("/setScrollBetweenEffectEnabled", HTTP_POST, handleSetScrollBetweenEffectEnabled); // Abilita/disabilita effetto tra scorrimenti
...
void handleRoot() {
    Serial.println("handleRoot chiamata");
    Serial.print("Heap libero: ");
    Serial.println(ESP.getFreeHeap());

    // Invia HTML in modalità chunked per risparmiare memoria
...
<head>
    ...
    // Parte 2: Body HTML
    server.sendContent(F(R"rawliteral(<body>
    <div class="container">
        <header>
            ...
            <div class="digital-timing-settings" style="margin-top:15px;padding:15px;background:rgba(255,255,255,0.1);border-radius:8px;">
                ...
                <div style="display:flex;gap:20px;flex-wrap:wrap;align-items:center;">
                    ...
                    <div>
                        ...
                        <input type="number" id="digitalDuration" min="10" max="59" value="10"
                               style="width:60px;padding:5px;border-radius:4px;border:none;"
                               onfocus="startEditingDigital()" onchange="scheduleEndEditingDigital()" oninput="startEditingDigital()">
                    </div>
        ...
        <div class="control-section">
            ...
            <div style="display:flex;gap:15px;align-items:center;margin-bottom:15px;flex-wrap:wrap;">
                <label style="color:#ccc;">Pausa tra passaggi (sec):</label>
                <input type="number" id="scrollPause" min="0" max="60" value="3"
                       style="width:60px;padding:5px;border-radius:4px;border:none;">
                <button class="btn btn-secondary" onclick="saveScrollPause()" style="padding:5px 10px;">Salva</button>
            </div>
            <div style="margin-bottom:15px;">
            ...
            <div style="display:flex;gap:15px;align-items:center;margin-bottom:15px;flex-wrap:wrap;">
                <label style="color:#ccc;">Durata orario dopo scorrimento (sec):</label>
                <input type="number" id="scrollClockDuration" min="1" max="60" value="5"
                       style="width:60px;padding:5px;border-radius:4px;border:none;">
                <button class="btn btn-secondary" onclick="saveScrollClockDuration()" style="padding:5px 10px;">Salva</button>
            </div>
            <div style="margin-bottom:15px;">
                <label style="color:#ccc;display:block;margin-bottom:8px;">🎭 Effetto tra scorrimenti:</label>
                <select id="scrollBetweenEffect" onchange="saveScrollBetweenEffect()" style="width:100%;padding:8px;border-radius:4px;border:1px solid rgba(255,255,255,0.3);background:rgba(255,255,255,0.1);color:white;font-size:1rem;">
                    <option value="9" style="background:#333;color:white;">Orologio Digitale</option>
                    <option value="3" style="background:#333;color:white;">Matrix</option>
                    <option value="4" style="background:#333;color:white;">Matrix2</option>
                    <option value="1" style="background:#333;color:white;">Slow</option>
                    <option value="0" style="background:#333;color:white;">Fade</option>
                    <option value="7" style="background:#333;color:white;">Galaga</option>
                    <option value="8" style="background:#333;color:white;">Pacman</option>
                    <option value="13" style="background:#333;color:white;">Arkanoid</option>
                    <option value="14" style="background:#333;color:white;">Natale 🎄</option>
                    <option value="15" style="background:#333;color:white;">Neve ❄️</option>
                    <option value="16" style="background:#333;color:white;">Laser 🔦</option>
                    <option value="17" style="background:#333;color:white;">Fuoco 🔥</option>
                    <option value="18" style="background:#333;color:white;">Black Hole 🌌</option>
                    <option value="19" style="background:#333;color:white;">Fire 1 🔥</option>
                    <option value="20" style="background:#333;color:white;">Firework 🎆</option>
                    <option value="21" style="background:#333;color:white;">Drift 🌊</option>
                    <option value="5" style="background:#333;color:white;">TRON</option>
                    <option value="11" style="background:#333;color:white;">Goccia</option>
                    <option value="6" style="background:#333;color:white;">Moto</option>
                    <option value="12" style="background:#333;color:white;">Rainbow</option>
                    <option value="2" style="background:#333;color:white;">Fast</option>
                </select>
                <label style="color:#ccc;display:flex;align-items:center;gap:8px;cursor:pointer;margin-top:10px;">
                    <input type="checkbox" id="scrollBetweenEffectEnabled" onchange="saveScrollBetweenEffectEnabled()" checked>
                    <span>✨ Abilita Effetto tra Scorrimenti</span>
                </label>
            </div>
            ...
            <button class="btn btn-primary" onclick="sendScrollText()">📡 Invia Testo Scorrevole</button>
        </div>
    </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Event listener per select effetto tra scorrimenti
        document.getElementById('scrollBetweenEffect').addEventListener('focus', startEditingScrollBetweenEffect);
        document.getElementById('scrollBetweenEffect').addEventListener('change', keepEditingScrollBetweenEffect);
        document.getElementById('scrollBetweenEffect').addEventListener('blur', scheduleEndEditingScrollBetweenEffect);
    ...
    function keepEditingDayNight() {
        ...
        editingTimeout = setTimeout(endEditingDayNight, 15000);
    }
    ...
    // Funzioni per gestire editing select effetto tra scorrimenti
    function startEditingScrollBetweenEffect() {
        userIsEditingScrollBetweenEffect = true;
        if (scrollBetweenEffectEditingTimeout) {
            clearTimeout(scrollBetweenEffectEditingTimeout);
            scrollBetweenEffectEditingTimeout = null;
        }
    }

    function keepEditingScrollBetweenEffect() {
        userIsEditingScrollBetweenEffect = true;
        if (scrollBetweenEffectEditingTimeout) {
            clearTimeout(scrollBetweenEffectEditingTimeout);
        }
        scrollBetweenEffectEditingTimeout = setTimeout(function() {
            userIsEditingScrollBetweenEffect = false;
            scrollBetweenEffectEditingTimeout = null;
        }, 10000);
    }

    function scheduleEndEditingScrollBetweenEffect() {
        if (scrollBetweenEffectEditingTimeout) {
            clearTimeout(scrollBetweenEffectEditingTimeout);
        }
        scrollBetweenEffectEditingTimeout = setTimeout(function() {
    ...
    function updateStatusIfNotEditing() {
        if (!userIsEditingDayNight && !userIsEditingScrollText && !userIsEditingScrollPause && !userIsEditingScrollBetweenEffect) {
            updateStatus();
        } else {
    ...
    function updateStatus() {
        fetch('/status')
            ...
            .then(function(data) {
                ...
                if (data.scrollClockDuration !== undefined) {
                    document.getElementById('scrollClockDuration').value = data.scrollClockDuration;
                }
                if (data.scrollBetweenEffect !== undefined && !userIsEditingScrollBetweenEffect) {
                    document.getElementById('scrollBetweenEffect').value = data.scrollBetweenEffect;
                }
                if (data.scrollBetweenEffectEnabled !== undefined) {
                    document.getElementById('scrollBetweenEffectEnabled').checked = data.scrollBetweenEffectEnabled;
                }
    ...
    // Parte 4: Altre funzioni JavaScript
    server.sendContent(F(R"rawliteral(
...
function sendScrollText() {
    ...
    // Permetti invio se c'è testo OPPURE se data/ora sono attivi
    if (txt.length === 0 && !showDate && !showTime) {
        showNotification("Inserisci un testo o attiva data/ora!", "error");
        return;
    }
...
function saveScrollClockDuration() {
    const duration = document.getElementById('scrollClockDuration').value;
    fetch('/setScrollClockDuration', {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
    ...
    .then(r => r.text())
    .then(resp => {
        showNotification("Durata orario salvata: " + duration + " sec", "success");
    })
    .catch(err => showNotification("Errore salvataggio durata orario", "error"));
}
...
function saveScrollOptions() {
    ...
    fetch('/setScrollOptions', {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: 'showText=' + showText + '&showDate=' + showDate + '&showTime=' + showTime
    })
    .then(r => r.text())
    .then(resp => {
        let msg = [];
...
function saveScrollBetweenEffect() {
    const effect = document.getElementById('scrollBetweenEffect').value;
    fetch('/setScrollBetweenEffect', {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: 'effect=' + effect
    })
    ...
    .then(resp => {
        ...
        const effectName = effectNames[effect] || 'Sconosciuto';
        showNotification("Effetto tra scorrimenti salvato: " + effectName, "success");
    })
    .catch(err => showNotification("Errore salvataggio effetto", "error"));
}

function saveScrollBetweenEffectEnabled() {
    const enabled = document.getElementById('scrollBetweenEffectEnabled').checked ? 1 : 0;
    fetch('/setScrollBetweenEffectEnabled', {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
    ...
    .then(r => r.text())
    .then(resp => {
        showNotification(enabled ? "Effetto tra scorrimenti abilitato" : "Effetto tra scorrimenti disabilitato", "success");
    })
    .catch(err => showNotification("Errore salvataggio impostazione", "error"));
}
...
function updateTotalCharCount() {
    ...
    // Data: es. "MERCOLEDI 04 DICEMBRE 2025" = ~26 caratteri (media)
    if (showDate) {
        total += 26;
        parts++;
    }
    // Ora: "HH:MM" = 5 caratteri
    if (showTime) {
        total += 5;
        parts++;
    }
...
function applyAdvancedColor() {
    ...
    Serial.println("HTML inviato (chunked)");
}
...
void handleStatus() {
    ...
    switch(currentPreset) {
        ...
        case 20: modeName = "Effetto Arkanoid"; break;
        case 21: modeName = "Effetto Natale"; break;
        case 22: modeName = "Effetto Neve"; break;
        case 23: modeName = "Effetto Laser"; break;
        case 24: modeName = "Effetto Fuoco"; break;
        case 25: modeName = "Effetto Black Hole"; break;
        case 26: modeName = "Fire 1"; break;
        case 27: modeName = "Effetto Firework"; break;
        case 28: modeName = "Effetto Drift"; break;
    ...
    doc["scrollClockDuration"] = scrollClockDuration;  // Durata orologio dopo scorrimento
    doc["scrollBetweenEffect"] = scrollBetweenEffect;  // Effetto da visualizzare tra gli scorrimenti
    doc["scrollBetweenEffectEnabled"] = scrollBetweenEffectEnabled;  // Abilita/disabilita effetto tra scorrimenti
    doc["scrollShowText"] = scrollShowText;
    doc["scrollShowDate"] = scrollShowDate;
    doc["scrollShowTime"] = scrollShowTime;
    doc["scrollText"] = customScrollText;  // Testo scorrevole memorizzato
    doc["rainbowMode"] = textRainbowMode;
    // Colore corrente in formato hex
    char colorHex[7];
...
void handleSetMode() {
    if (server.hasArg("mode")) {
        ...
        if (newMode < NUM_PRESETS) { // 24 preset totali (0-23)
            currentPreset = newMode;
            applyPreset(currentPreset);
            // NON caricare impostazioni salvate - usa sempre i colori di default del preset
            // L'utente può cambiare colore temporaneamente, ma premendo il pulsante torna al default
...
void handleSetColor() {
    if (server.hasArg("color")) {
        ...
        if (colorHex.length() == 6) {
            ...
            // Salva in EEPROM per preset DIGITALE (18) e SCROLLTEXT (19)
            if (currentPreset == 18 || currentPreset == 19) {
                saveModeSettings(currentPreset);
                Serial.print("Colore salvato in EEPROM per preset ");
...
void handleSetRainbow() {
    ...
    if (currentPreset == 18 || currentPreset == 19) {
        saveModeSettings(currentPreset);
        Serial.print("Rainbow salvato in EEPROM per preset ");
        Serial.println(currentPreset);
    }
...
// Nuova funzione per gestire la durata dell'orologio dopo scorrimento
void handleSetScrollClockDuration() {
    if (server.hasArg("duration")) {
        uint8_t duration = server.arg("duration").toInt();
        if (duration < 1) duration = 1;
        if (duration > 60) duration = 60;
        
        scrollClockDuration = duration;

        // Salva in EEPROM
        EEPROM.write(EEPROM_SCROLL_CLOCK_DURATION_ADDR, scrollClockDuration);
        EEPROM.commit();

        Serial.print("Durata orologio dopo scorrimento impostata a: ");
        Serial.print(scrollClockDuration);
...
void handleSetScrollSpeed() {
    if (server.hasArg("speed")) {
        ...
        EEPROM.write(EEPROM_SCROLL_SPEED_ADDR + 1, (speed >> 8) & 0xFF); // Byte alto
        EEPROM.commit();
...
// Nuova funzione per gestire l'effetto da visualizzare tra gli scorrimenti
void handleSetScrollBetweenEffect() {
    if (server.hasArg("effect")) {
        uint8_t effect = server.arg("effect").toInt();
        if (effect < NUM_MODES) {
            scrollBetweenEffect = effect;
            
            // Salva in EEPROM
            EEPROM.write(EEPROM_SCROLL_BETWEEN_EFFECT_ADDR, scrollBetweenEffect);
            EEPROM.commit();
            
            Serial.print("Effetto tra scorrimenti impostato a: ");
...
// Funzione per abilitare/disabilitare l'effetto tra scorrimenti
void handleSetScrollBetweenEffectEnabled() {
    if (server.hasArg("enabled")) {
        scrollBetweenEffectEnabled = (server.arg("enabled").toInt() == 1);

        // Salva in EEPROM
        EEPROM.write(EEPROM_SCROLL_BETWEEN_EFFECT_ENABLED_ADDR, scrollBetweenEffectEnabled ? 1 : 0);
        EEPROM.commit();

        Serial.print("Effetto tra scorrimenti ");
        Serial.println(scrollBetweenEffectEnabled ? "abilitato" : "disabilitato");
...
void handleSetScrollOptions() {
    if (server.hasArg("showText") || server.hasArg("showDate") || server.hasArg("showTime")) {
        ...
        // Reinizializza il testo scorrevole per applicare le nuove opzioni
        scrollTextInitialized = false;
...
void handleSetScrollText() {
    if (server.hasArg("text")) {
        ...
        currentPreset = 19;  // Preset per testo scorrevole
        ...
        // Salva anche il preset corrente
        EEPROM.write(EEPROM_PRESET_ADDR, currentPreset);
        EEPROM.commit();
...
void setup() {
   ...
   if (EEPROM.read(0) != EEPROM_CONFIGURED_MARKER) {
       ...
       // Valori default per testo scorrevole
       ...
       EEPROM.write(EEPROM_SCROLL_CLOCK_DURATION_ADDR, 5); // durata orologio 5 secondi
       EEPROM.write(EEPROM_SCROLL_BETWEEN_EFFECT_ADDR, MODE_DIGITALE); // default: orologio digitale
       EEPROM.write(EEPROM_SCROLL_BETWEEN_EFFECT_ENABLED_ADDR, 1); // effetto tra scorrimenti abilitato di default
   ...
   // Carica velocità testo scorrevole (2 byte)
   scrollTextSpeedMs = EEPROM.read(EEPROM_SCROLL_SPEED_ADDR) | (EEPROM.read(EEPROM_SCROLL_SPEED_ADDR + 1) << 8);
   ...
   // Carica durata orologio dopo scorrimento
   scrollClockDuration = EEPROM.read(EEPROM_SCROLL_CLOCK_DURATION_ADDR);
   if (scrollClockDuration < 1 || scrollClockDuration > 60) {
       scrollClockDuration = 5; // default 5 secondi
   }
   
   // Carica effetto da visualizzare tra gli scorrimenti
   scrollBetweenEffect = EEPROM.read(EEPROM_SCROLL_BETWEEN_EFFECT_ADDR);
   if (scrollBetweenEffect >= NUM_MODES) {
       scrollBetweenEffect = MODE_DIGITALE; // Default: orologio digitale
   }

   // Carica flag abilitazione effetto tra scorrimenti
   scrollBetweenEffectEnabled = (EEPROM.read(EEPROM_SCROLL_BETWEEN_EFFECT_ENABLED_ADDR) == 1);
   // Se EEPROM non inizializzata, default true
   if (EEPROM.read(EEPROM_SCROLL_BETWEEN_EFFECT_ENABLED_ADDR) == 255) scrollBetweenEffectEnabled = true;

   // Validazione valori caricati
   ...
   Serial.println(" ms");
   Serial.print("Durata orologio dopo scorrimento: ");
   Serial.print(scrollClockDuration);
   Serial.println(" secondi");
   Serial.print("Effetto tra scorrimenti: ");
   Serial.println(scrollBetweenEffect);
   Serial.println("=====================================");
   ...
   // Carica impostazioni specifiche per la modalità (colore, rainbow) DOPO applyPreset
...
bool effectCycleCompleted() {
    ...
    switch (currentMode) {
        ...
        case MODE_SCROLLTEXT:
            // Testo scorrevole: mai "completato", è continuo
            return false;
        case MODE_DIGITALE:
            // Già in digitale, non serve overlay
            return false;
        case MODE_FIRE1:
            // Effetto FIRE1: dura esattamente 15 secondi
            return (now - lastEffectChangeTime >= FIRE1_DURATION_MS);
        case MODE_FIREWORK:
            // Effetto FIREWORK: completato quando fireworkCompletionTime != 0
            return (fireworkCompletionTime != 0);
        case MODE_DRIFT:
            // Effetto DRIFT: completato quando driftCompletionTime != 0
...
// Funzione per riavviare gli effetti speciali completati ogni 25 secondi
void restartCompletedEffects() {
    static uint32_t lastEffectRestart = 0;
    const uint32_t EFFECT_RESTART_INTERVAL = 25000; // 25 secondi
    
    uint32_t now = millis();
    
    if (now - lastEffectRestart >= EFFECT_RESTART_INTERVAL) {
        lastEffectRestart = now;
        
        // Riavvia l'effetto corrente se è un effetto speciale e è completato
        switch (currentMode) {
            case MODE_MOTO:
                if (motoCompleted) {
                    initMotoEffect();
...
void loop() {
    ...
    restartCompletedEffects(); // Riavvia effetti speciali ogni 25 secondi
   ...
   if(WiFi.status() == WL_CONNECTED && currentMillis - lastUpdate > 1000) {
       ...
       if(newMinute != currentMinute || newHour != currentHour) {
           ...
           fireworkEffectActive = false;
           driftEffectActive = false;
           resetScrollText();  // Reset testo scorrevole
           ...
           if (currentMode == MODE_ARKANOID) {
               initArkanoidEffect();
           }
           if (currentMode == MODE_CHRISTMAS) {
               initChristmasEffect();
           }
           if (currentMode == MODE_SNOWFALL) {
               initSnowfallEffect();
           }
           if (currentMode == MODE_LASER) {
               initLaserEffect();
           }
           if (currentMode == MODE_FUOCO) {
               initFuocoEffect();
           }
           if (currentMode == MODE_BLACKHOLE) {
               initBlackHoleEffect();
           }
           if (currentMode == MODE_FIRE1) {
               initFire1Effect();
           }
           if (currentMode == MODE_FIREWORK) {
               initFireworkEffect();
           }
...
void applyPreset(uint8_t preset) {
   ...
   switch(preset) {
       ...
       case 19:
           // Testo scorrevole - il testo è già caricato da EEPROM
           currentMode = MODE_SCROLLTEXT;
           manualModeSelected = true;  // Non interrompere con overlay
           resetScrollText();  // Forza reinizializzazione
           break;
...
void updateFadeEffect() {
    ...
    // Se l'effetto è completato, mostra l'orario fisso con i secondi lampeggianti
    if (fadeState.completed) {
        FastLED.clear();
        displayWord(WORD_SONO_LE, currentColor);
        displayWord(hourWord, currentColor);
...
void updateSlowEffect() {
    ...
    if (slowState.holdPhase) {
        // Mostra orario con secondi anche durante hold
        FastLED.clear();
        displayWord(WORD_SONO_LE, slowState.sonoColor);
        displayWord(hourWord, slowState.hourColor);
        if (currentMinute > 0) {
...
void updateTronEffect() {
    ...
    // Inizializza moto se necessario
    if(tronNeedsInit) {
        initTronBikes();
    }
...
void updateDropEffect() {
    ...
    // Se completato, mostra orario fisso con secondi
    if (dropState.completed) {
        FastLED.clear();
...
void updateMotoEffect() {
    ...
    // Se completato, mostra orario fisso con secondi lampeggianti
    if (motoCompleted) {
